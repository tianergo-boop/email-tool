...
class Theme:
    PRIMARY      = "#2B5AED"   # 主色 - 专业蓝
    PRIMARY_HOVER= "#1E45C0"   # 主色悬停
    SUCCESS      = "#52C41A"   # 成功绿
    WARNING      = "#FAAD14"   # 警告黄
    ERROR        = "#F5222D"   # 错误红
    BG_PAGE      = "#F5F7FA"   # 页面背景
...
class EmailToolApp:
    ...
    def _setup_styles(self):
        ...
        style.map("Primary.TButton",
                  background=[("active", Theme.PRIMARY_HOVER), ("pressed", "#1635A1")])
    ...
    def _build_file_card(self, parent):
        ...
        for i, (label, var, cmd, hint) in enumerate(rows):
            ...
            entry = ttk.Entry(inner, textvariable=var, style="Input.TEntry", width=56)
    ...
    def _build_smtp_card(self, parent):
        ...
        entry_pass = ttk.Entry(inner, textvariable=self.email_pass, style="Input.TEntry", width=18, show="•")
        entry_pass.grid(row=r, column=3, sticky="w", padx=(8, 0), pady=6)
    ...
    def _build_log_area(self, parent):
        ...
        self.log_text.tag_configure("info",    foreground=Theme.PRIMARY)
    ...
    def do_task(self):
        ...
        try:
            ...
            # Step 0：刷新透视表
            self.log("🔄 尝试刷新 Excel 数据透视表…", "info")
            refreshed = refresh_pivot_tables(excel_path)
            if refreshed:
                self.log("   ✅ 透视表已刷新", "success")
            else:
                self.log("   ⚠️  无法自动刷新透视表（仅 Windows + Excel 环境支持）", "warning")
                self.log("       请确保 Excel 文件中的透视表已手动刷新！", "warning")
            ...
            self.log(f"   拆分完成，共 {len(pdf_map)} 个订单 PDF", "success")
            ...
            for i, item in enumerate(email_data, 1):
                ...
                if coa_ok:
                    ...
                    self.log(f"   ✅ COA 打包：{len(coa_files)} 个文件", "success")
                else:
                    self.log(f"   ⚠️  未找到 COA 文件", "warning")
            ...
            self.log(f"🎉 全部完成！成功 {ok_cnt} 封，失败 {err_cnt} 封",
                     "success" if err_cnt == 0 else "warning")
            self.log(f"📁 输出文件在：{work_dir}")
            self.status_label.config(text=f"  ✅ 执行完成 — 成功 {ok_cnt} 封，失败 {err_cnt} 封")

        except Exception as e:
            self.log(f"\n❌ 执行出错：{e}", "error")
            import traceback
            self.log(traceback.format_exc())
            self.status_label.config(text=f"  ❌ 执行出错")
